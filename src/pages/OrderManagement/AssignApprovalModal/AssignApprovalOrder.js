...
const {Option} = Select
export default function ModalAssignApprovalOrder({modalAssignApproval, setModalAssignApproval, refetch}) {
    ...
    const {data: resAccount} = UserApi.getUserByRole({
            role: ""
        }
    ...
    const handleAssignApproval = () => {
        ...
        OrderApi.assignApproval({...formCreate, orderCodeList: modalAssignApproval?.orderCodeList}).then((res) => {
                refetch()
