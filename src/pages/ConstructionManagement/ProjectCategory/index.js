...
export default function ProjectCategory() {
    ...
    const {data, refetch} = ProjectCategoryApi.useGetProjectCategories(formSearch, {staleTime: 0, cacheTime: 0})
    useEffect(() => {
        if (modalCreate.code) {
        ...
        return (
            <div className="p-1 pointer">
                <div className="mb-0 p-2 pr-6 hover:bg-red-100"
                     onClick={() => setModalCreate({status: true, code: record?.code, type: "update"})}>
                    Chỉnh sửa
                </div>
                <Popconfirm
                    title="Thông báo"
    ...
    const handleSubmit = async () => {
        ...
        if (modalCreate.code) {
            await ProjectCategoryApi.update(formCreate).then(() => {
                refetch()
                closeModal()
                AppNotification.success("Cập nhật hạng mục thành công");
            }).catch((error) => {
            ...
            await ProjectCategoryApi.create(formCreate).then(() => {
                refetch()
                closeModal()
                AppNotification.success("Thêm mới hạng mục thành công");
            }).catch((error) => {
                const errorCode = error.errorCode;
                if (errorCode?.includes(errorCodes.NAME_EXIST)) {
                    setFormErrors({...formErrors, name: errorTexts.DATA_EXIST})
    ...
    const handleDelete = async (record) => {
        ...
        await ProjectCategoryApi.delete({"code": record.code}).then(() => {
            refetch()
            AppNotification.success("Xóa hạng mục thành công");
            closeModal()
        }).catch((error) => {
    ...
    return (
        <React.Fragment>
            <div className="m-[20px] flex">
                <AppCreateButton text={"Thêm mới"} onClick={() => setModalCreate({status: true})}/>
            ...
            <div className={`pl-4 pr-4`}>
                ...
                <Modal
                    title={` ${
                        modalCreate.type === "detail" ? "Chi tiết" : (modalCreate.id ? "Cập nhật" : "Thêm mới")
                    } hạng mục`}
                    ...
                    <Form name="validateOnly" layout="vertical" autoComplete="off">
                        ...
                        <Form.Item label="Mô tả">
                            <Checkbox checked={formCreate?.status === "ACTIVE"}
                                      onChange={(value) => modalCreate?.type !== "detail" ? handleFormUpdate(setFormCreate, setFormErrors, "status", value.target.checked ? "ACTIVE" : "INACTIVE") : null}
                            >
                                Hoạt động
                            </Checkbox>
                        </Form.Item>
